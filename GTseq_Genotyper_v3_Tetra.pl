#!/usr/bin/perl
#GTseq_Genotyper_v3_Tetra.pl by Nate Campbell
#Genotype GTseq libraries from individual .fastq files **Modified version for genotyping tetraploid species**
...
#Fix allele counts to non-zero number for division ratio calculation...
	...
	if ($Allele1_Count{$loci} + $Allele2_Count{$loci} < 20) {$geno = "0000"; $genoclass = "NA";} #Set genotypes of low allele count loci to "00"
	elsif ($ratio >= 19) {$geno = "$allele1name{$loci}$allele1name{$loci}$allele1name{$loci}$allele1name{$loci}"; $genoclass = "A1HOM"; #Allele1 Homozygotes (0% a2)
		$HOM_CT = $HOM_CT + $Allele1_Count{$loci}; $BKGRD_CT = $BKGRD_CT + $Allele2_Count{$loci};}
	elsif ($ratio > 5.666) {$geno = "0000"; $genoclass = "NA"; $HOM_CT = $HOM_CT + $Allele1_Count{$loci}; $BKGRD_CT = $BKGRD_CT + $Allele2_Count{$loci};} #Inbetween A1HOM and HET1 genotype classes...
	elsif ($ratio > 1.85) {$geno = "$allele1name{$loci}$allele1name{$loci}$allele1name{$loci}$allele2name{$loci}"; $genoclass = "HET1";} #First heterozygote category (25% a2)
	elsif ($ratio > 1.5) {$geno = "0000"; $genoclass = "NA";} #Inbetween HET1 and HET2 genotype classes...
	elsif ($ratio > 0.666) {$geno = "$allele1name{$loci}$allele1name{$loci}$allele2name{$loci}$allele2name{$loci}"; $genoclass = "HET2";} #Second heterozygote category (50% a2)
	elsif ($ratio > 0.538) {$geno = "0000"; $genoclass = "NA";} #Inbetween HET2 and HET3 genotype classes...
	elsif ($ratio > 0.176) {$geno = "$allele1name{$loci}$allele2name{$loci}$allele2name{$loci}$allele2name{$loci}"; $genoclass = "HET3";} #Third heterozygote category (75% a2)
	elsif ($ratio > 0.0526) {$geno = "0000"; $genoclass = "NA"; $HOM_CT = $HOM_CT + $Allele2_Count{$loci}; $BKGRD_CT = $BKGRD_CT + $Allele1_Count{$loci};} #Inbetween HET3 and A2HOM genotype classes...
	elsif ($ratio <= 0.1) {$geno = "$allele2name{$loci}$allele2name{$loci}$allele2name{$loci}$allele2name{$loci}"; $genoclass = "A2HOM"; #Allele2 Homozygotes (100% a2)
...
#Calculate the presence of possible null alleles using fuzzy match ratio...
	...
	if (($genoclass !~ "HET[123]") && ($fuzz_ratio > 0.8) && ($sum_xy > 10)) {$Null_Error{$loci} = "Fail"}
